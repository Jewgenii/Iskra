<%-- 
    Document   : searchViewTmpK
    Created on : 30.03.2018, 8:29:55
    Author     : u27brvz18
--%>
    ...
    <body>
            ...
            <div class="panel panel-default">
                ...
                <div id="collapse1" class="panel-collapse collapse in">
                    <div class="panel-body">
                        <div class="container-fluid">
                            <div class="row">
                                <form  class="search pull-left" style="padding-right: 5px;" method="get" name="frm" action="SearchTmpK">
                                    ...
                                    <!--<input id="searchClear" class="btn btn-warning btn-md" type="button" value="Відмінити фільтр" onclick="window.location.href = 'TmpsqlKController?action=list&page=1'" />-->
                                </form>

                                <a class="pull-left" href="SearchTmpK?page=1&count=0&osdch=&osdk=&cp=&svi=<%=svi%>">
        ...
        <div class="container-fluid">
            <nav class="pag" role='pagination'>
                <ul class="pagination">
                    <c:choose>
                        <c:when test="${page == 1}"><li><a class="disabled" href="SearchTmpK?page=1&count=0&osdch=${osdch}&osdk=${osdk}&cp=${cp}&svi=${svi}">Перша</a></li></c:when> 
                        <c:otherwise><li><a class="a2" href="SearchTmpK?page=1&count=0&osdch=${osdch}&osdk=${osdk}&cp=${cp}&svi=${svi}">Перша</a></li></c:otherwise>
                        </c:choose>

                    <c:choose>
                        <c:when test="${page <=1}"><li class="li1"><a class="disabled" href="SearchTmpK?page=${page-1}&count=${count-100}&osdch=${osdch}&osdk=${osdk}&cp=${cp}&svi=${svi}"><span class="glyphicon glyphicon-chevron-left"></span></a></li></c:when> 
                        <c:otherwise><li class="li1"><a class="a2" href="SearchTmpK?page=${page-1}&count=${count-100}&osdch=${osdch}&osdk=${osdk}&cp=${cp}&svi=${svi}"><span class="glyphicon glyphicon-chevron-left"></span></a></li></c:otherwise> 
                            </c:choose>
                    ...
                    <c:forEach var="i" begin="${ firstPageNum > 0 ? firstPageNum : 1 }" 
                        ...
                        <c:choose>
                            <c:when test="${i == page}"><li class="li1"><a class="a2" style="color: #2e6da4;"  href="SearchTmpK?page=${i}&count=${(i-1)*100}&osdch=${osdch}&osdk=${osdk}&cp=${cp}&svi=${svi}"><b>${i}</b></a></li></c:when> 
                            <c:otherwise> <li class="li1"><a class="a2" href="SearchTmpK?page=${i}&count=${(i-1)*100}&osdch=${osdch}&osdk=${osdk}&cp=${cp}&svi=${svi}">${i}</a></li></c:otherwise> 
                            </c:choose>     
                        </c:forEach>

                    <c:choose>
                        <c:when test="${page >= size}"><li class="li1"><a class="disabled" href="SearchTmpK?page=${page+1}&count=${count+100}&osdch=${osdch}&osdk=${osdk}&cp=${cp}&svi=${svi}"><span class="glyphicon glyphicon-chevron-right"></span></a></li></c:when> 

                        <c:otherwise><li><a class="a2" href="SearchTmpK?page=${page+1}&count=${count+100}&osdch=${osdch}&osdk=${osdk}&cp=${cp}&svi=${svi}"><span class="glyphicon glyphicon-chevron-right"></span></a></li></c:otherwise>
                            </c:choose>
